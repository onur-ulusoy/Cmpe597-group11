...
def clean_qwen_output(text, valid_classes=VALID_CLASSES):
    """
    Robustly extract exactly one valid 3-class polarity label.

    Returns:
        "Positive", "Negative", "Neutral", or "INVALID"
    """
    ...
    text = re.sub(r"^\s*(sentiment|polarity|emotion|label|answer)\s*:\s*", "", text, flags=re.IGNORECASE)
    lowered = text.lower().strip()

    # Exact label after removing punctuation/spaces.
...
def build_prompt(caption, annotation_mode):
    """
    Build a 3-class sentiment/polarity prompt.

    The image_caption mode treats caption as primary evidence and image as supporting context.
    This reduces wrong labels caused by literal visual cues.
    """
    if annotation_mode == "caption_only":
        return f"""You are annotating the dominant sentiment polarity of a meme caption.

The caption is a human-written explanation of the meme poster's intended meaning.
Choose exactly one label: Positive, Negative, or Neutral.

Label definitions:

Positive:
The meme is mainly playful, funny, affectionate, admiring, satisfying, celebratory, or light-hearted.

Negative:
The meme mainly expresses criticism, anger, frustration, sadness, disappointment, disgust, fear, worry, rejection, or something being bad or ruined.

Neutral:
The meme is mostly descriptive, unclear, mixed, weakly emotional, observational, or the sentiment cannot be confidently determined.

Important rules:
- Do not label every joke as Positive. If the joke is mainly criticizing, complaining, or expressing disappointment, choose Negative.
- If the meme is harmless absurd humor, playful irony, or self-deprecating humor without strong negativity, choose Positive.
- If positive and negative signals are balanced, choose Neutral.
- If the caption is only explaining a situation without clear emotion, choose Neutral.
...
Label:"""
    ...
    if annotation_mode == "image_caption":
        return f"""You are annotating the dominant sentiment polarity of an internet meme.
...
- If the meme is harmless absurd humor, playful irony, or self-deprecating humor without strong negativity, choose Positive.
- If positive and negative signals are balanced, choose Neutral.
- Do not label based only on literal visual features such as fire, weapons, crying faces, angry faces, scared faces, dark colors, or dramatic scenes.
- Use the image only to clarify the meaning of the caption.
...
def build_retry_prompt(caption, annotation_mode):
    ...
    return f"""Classify the dominant sentiment polarity of the {input_description}.

Rules:
- Positive: playful, funny, affectionate, admiring, satisfying, celebratory, or light-hearted.
- Negative: criticism, complaint, anger, frustration, sadness, disappointment, disgust, fear, worry, rejection, or something bad/ruined.
- Neutral: descriptive, unclear, mixed, weakly emotional, or balanced positive and negative signals.
...
def compute_distribution(items, label_key):
    ...
    aux_or_missing_total = counter.get("INVALID", 0) + counter.get("MISSING", 0)
...
def save_markdown_report(samples, image_root, output_path, label_key, raw_key, annotation_mode):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(f"# Qwen-VL 3-Class Manual Quality Check - {annotation_mode}\n\n")
...
def process_split(split_name, input_path, model, tokenizer, args, output_base):
    ...
    label_key = f"qwen_3class_{args.annotation_mode}_sentiment_label"
    raw_key = f"qwen_3class_{args.annotation_mode}_raw_response"
    ...
    for item in tqdm(data, desc=f"{split_name} / 3class / {args.annotation_mode}"):
        caption = get_meme_caption(item)
        ...
        try:
            ...
            final_label = clean_qwen_output(raw_response)
            ...
            if final_label == "INVALID" and args.retry_invalid:
                ...
                retry_label = clean_qwen_output(retry_response)

                item[f"qwen_3class_{args.annotation_mode}_first_raw_response"] = raw_response
                item[f"qwen_3class_{args.annotation_mode}_retry_raw_response"] = retry_response
        ...
        item[f"qwen_3class_{args.annotation_mode}_model"] = args.model_name
        item[f"qwen_3class_{args.annotation_mode}_annotation_mode"] = args.annotation_mode

        processed_data.append(item)

    output_json = output_base / f"{split_name}_qwen_3class_{args.annotation_mode}_labels.json"
    ...
    report_json = output_base / f"{split_name}_qwen_3class_{args.annotation_mode}_imbalance_report.json"
...
def main(args):
    ...
    if args.annotation_mode == "caption_only":
        run_slug = f"{model_slug}_3class_caption_only"
    elif args.annotation_mode == "image_caption":
        run_slug = f"{model_slug}_3class_image_caption"
    else:
    ...
    combined_report_path = output_base / f"qwen_3class_{args.annotation_mode}_combined_imbalance_report.json"
    ...
    if args.manual_per_class > 0 and all_processed_data:
        ...
        manual_check_path = output_base / f"manual_check_3class_{args.annotation_mode}_stratified.md"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Example: --output_suffix test creates Qwen_VL_Chat_3class_image_caption_test."
        ),
    )
